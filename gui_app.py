...
class MainWindow(QtWidgets.QWidget):
        ...
        if event.type() == QtCore.QEvent.KeyPress:
            ...
            if focused_widget == self.start_key_assign:
                    if self.check_valid(text):
                        ...
                        self.update_hotkeys()
                    return True  
            elif focused_widget == self.stop_key_assign:
                    if self.check_valid(text):
    ...
    def style_text(self):
        ...
        fixed = [self.bpm_input, self.channel_input, self.select_chord, self.select_mode, self.stop_key_assign, self.start_key_assign]
    ...
    def setup_layout(self):
        ...
        layout.addWidget(self.textEdit, 8, 1, alignment=QtCore.Qt.AlignBottom)
    ...
    def open_file(self):
        ...
        if file_dialog.exec():
            ...
            self.textEdit.clear()
            self.textEdit.setText(selected_files[0])
    ...
    def play_arguments(self):
        smidi_path = self.textEdit.text().strip()
        ...
        if path.isfile('./config.ini'):
            try:
                ...
                if config.has_section('midi-to-appeal'):
                    stop_key = config.get('midi-to-appeal', 'stop_key', fallback='F5')
                    start_key = config.get('midi-to-appeal', 'start_key', fallback='F4')
